<#
.SYNOPSIS
Updates a Spotify playlist with new releases from followed artists and maintains the playlist by removing older tracks.

.DESCRIPTION
This script retrieves new releases from artists followed by the user on Spotify and adds them to a specified playlist. 
...
.LINK
https://gist.github.com/TinkerHack/91ad6d919475bb0a008458c68f844b6e
...
8. You're now ready to run the script in Powershell cmd using the following command (and default values):
   ...
   You can add more parameters according to your needs. See ".PARAMETER" section above for details or read the comments for each parameter below.
#>
[CmdletBinding()]
param (
...
# Set log path file 
[string]$LastRunDateFilePath = (Join-Path $PSScriptRoot "Get-FollowedArtistsReleases_LastRunDate.txt")

# Start Transcript
$transcriptPath = Join-Path $PSScriptRoot "Get-FollowedArtistsReleases_Log.txt"
...
function Get-FilteredFollowedArtists {
    param (
        [Parameter(Mandatory=$false)]
        [string[]]$ExcludeArtists,
        [Parameter(Mandatory=$false)]
        [string]$SpotishellAppName
    )
    
    try {
        Write-Debug "Getting followed artists with application name: $SpotishellAppName"
        $artists = Get-FollowedArtists -ApplicationName $SpotishellAppName
        Write-Debug "Found $($artists.count) artists initially"
        
        if ($ExcludeArtists -and $ExcludeArtists.Count -gt 0) {
            Write-Debug "Filtering out excluded artists: $($ExcludeArtists -join ', ')"
            $artists = $artists | Where-Object { $_.name -notin $ExcludeArtists }
            Write-Debug "$($artists.count) artists remain after exclusion"
        }
...
# Helper function to update followed artists
function Update-FollowedArtistsReleases {
    param (
        [Parameter(Mandatory=$true)]
    ...
    try {        
        ...
        $artists = Get-FilteredFollowedArtists -ExcludeArtists $ExcludeArtistNames -SpotishellAppName $SpotishellAppName

        # If no artists found, throw an error
        if (-not $artists) {
            throw "No artists found or error occurred while fetching artists."
        }

        # Write warning if there are a lot of artists followed
        If($artists.Count -ge 500){
            Write-warning "Your following a lot of artists ($($artists.Count)), which is great, but might also cause throttling issues."
        }
        
        # Select the first 20 followed artists, used when troubleshooting, but could also be used for throttling issues.
        #$artists = $artists | Select-Object -First 20 # Todo: comment out when finished!
        ...
        $errorDetails = [string]$error[0]
        $errorLine = [string]$error[0].InvocationInfo.PositionMessage
        throw "An error occurred during Update-FollowedArtistsReleases at line $($errorLine): $errorDetails"
    }
}
...
try {
    ...
    Update-FollowedArtistsReleases `
